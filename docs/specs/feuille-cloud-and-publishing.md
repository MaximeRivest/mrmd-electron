# Feuille: Cloud, Publishing, and Collaboration

> MRMD as a cloud platform (feuille.dev), continuous publishing, real-time collaboration, and the data service architecture. Builds on the FSML spec and cloud-runtime-spec.
...
Feuille (feuille.dev) is MRMD running as a cloud service. Users sign in, get an isolated Linux environment, and access the full MRMD editor from any browser. Three capabilities layer on top of the core editor:
...
> Full details in `feuille-build-plan.md`. Summary of key decisions here.
...
```yaml config
publish:
  url: "@maxime/my-analysis"      # feuille.dev/@maxime/my-analysis
...
│──────────────│
│ Edit on      │  → opens feuille.dev editor (if user has access)
│ feuille ✎      │
└──────────────┘
```
...
```
Request: feuille.dev/@maxime/my-analysis/getting-started/setup
...
Exact same rendering as feuille.dev published pages.
...
On feuille.dev, the only GitHub integration is clone + open:
...
feuille.dev:
  ...
  3. Redirect to feuille.dev/someone/cool-project (editor view)
  Done.
```
...
| **On feuille.dev** | Full MRMD editor + published site |
...
│  Share                                           │
│                                                  │
│  feuille.dev/@maxime/my-analysis                   │
│                                                  │
│  People:                                         │
│    maxime (you)                    Owner          │
...
│                                                  │
│  ── Or share link ──────────────────────────     │
│  🔗 feuille.dev/join/a7f3b2c1     [Copy]          │
...
Gets URL: https://abc123.feuille.dev (tunneled)
...
### 7.1 feuille.dev URLs

| URL pattern | What it is |
|---|---|
| `feuille.dev/login` | OAuth sign-in |
| `feuille.dev/user/repo` | Clone + open a GitHub repo in editor |
| `feuille.dev/@user/project` | Published site (reader mode) |
| `feuille.dev/@user/project/path` | Published page (reader mode) |
| `feuille.dev/join/abc123` | Collaboration invite link |
| `user.feuille.dev` | Custom subdomain for published sites (Pro+) |
...
Where you edit it:        feuille.dev/maxime/my-analysis
Where the public reads:   feuille.dev/@maxime/my-analysis
...
```
1. Visit feuille.dev → Sign in with Google/GitHub
...
```
1. Paste github.com/someone/project into feuille
...
│  Published pages: feuille.dev/@user/project                          │
└──────────────────────────────┬───────────────────────────────────────┘
                               │
┌──────────────────────────────┴───────────────────────────────────────┐
│            Base Server — EC2 t4g.small (ca-central-1)                │
│                                                                       │
│  Caddy (reverse proxy, auto-HTTPS for feuille.dev)                   │
│  ┌──────────────┐  ┌──────────────┐  ┌────────────────┐             │
