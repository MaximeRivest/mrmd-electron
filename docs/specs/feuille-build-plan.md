...
> Bottom-up layered approach to building the full feuille.dev platform. Each layer has clear contracts, independent validation, and no integration surprises.
...
Route 53
└── feuille.dev → EC2 (editor/API) + CloudFront (static)
```

**Cost:** ~$15-20 CAD/mo (t3.small) + CloudFront free tier (1 TB/mo)

Domain: **feuille.dev** (registered, DNS managed by owner)
...
# 7. Caddy auto-HTTPS
caddy reverse-proxy --from feuille.dev --to localhost:8080

# 8. Postgres
sudo -u postgres createdb feuille
psql feuille -c "SELECT 1"
...
# → { "token": "a7f3b2c1", "url": "feuille.dev/join/a7f3b2c1" }
...
```
feuille.dev/@*           → publish-service:3003
feuille.dev/auth/*       → auth-service:3001
feuille.dev/join/*       → auth-service:3001 → then redirect to editor
feuille.dev/user-abc/*   → editor container on port 8081
feuille.dev/user-def/*   → editor container on port 8082
```
...
| Published site indicator | mrmd-electron index.html | Show "Live at feuille.dev/@user/project" when publish: configured |
